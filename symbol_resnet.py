...
from symdata.coordinate_convert import back_forward_convert
...
def get_resnet_train(anchor_scales, anchor_ratios, rpn_feature_stride,
    ...
    #第二步：rpn分类和回归，此处没有 rpn_bbox_pred_r函数
    # rpn feature
    ...
    #第三步：rpn的loss=cls loss+l1 loss
    # rpn bbox_h regression
...
################################此处有问题
    ...
    #第五步：rcnn roi pool
    # rcnn roi pool
    roi_pool = mx.symbol.ROIPooling(name='roi_pool', data=conv_feat, rois=rois, pooled_size=rcnn_pooled_size, spatial_scale=1.0 / rcnn_feature_stride)
    ...
    #第七步：rcnn bbox回归=旋转回归
    # rcnn bbox_r regression
    bbox_pred=mx.sym.FullyConnected(name='bbox_pred',data=top_feat,num_hidden=num_classes*5)
    bbox_loss_=bbox_weight*mx.symbol.smooth_l1(name='bbox_loss_',scalar=1.0,data=(bbox_pred-bbox_target))
    bbox_loss=mx.symbol.MakeLoss(name='bbox_loss',data=bbox_loss_,grad_scale=1.0/rcnn_batch_rois)
    # reshape output
    ...
    bbox_loss=mx.symbol.Reshape(data=bbox_loss,shape=(rcnn_batch_size,-1,5*num_classes),name='bbox_loss_reshape')
    # group output
...
def get_resnet_test(anchor_scales, anchor_ratios, rpn_feature_stride,
    ...
    bbox_pred=mx.symbol.FullyConnected(name='bbox_pred',data=top_feat,num_hidden=num_classes*5)
    # reshape output
    cls_prob=mx.symbol.Reshape(data=cls_prob,shape=(rcnn_batch_size,-1,num_classes),name='cls_prob_reshape')
    bbox_pred=mx.symbol.Reshape(data=bbox_pred,shape=(rcnn_batch_size,-1,5*num_classes),name='bbox_pred_rshape')
