...
def get_image(roi_rec, short, max_size, mean, std):
    ...
    roi_rec should have keys: ["image", "boxes", "gt_classes", "flipped"]   boxes:four rows.the proposal.left-up,right-down
    0 --- x (width, second dim of im)
    |
    y (height, first dim of im)
    """
    im = imdecode(roi_rec['image'])
    #print ('roi_rec[image]=',roi_rec['image'])
    if roi_rec["flipped"]:
        im = im[:, ::-1, :]
    ...
    #
    #修改：gt_boxes_and_labels(x1,y1,x2,y2,x3,y3,x4,y4,)转换成（x,y,w,h,theta,)
    if roi_rec['gt_classes'].size > 0:
        gt_inds = np.where(roi_rec['gt_classes'] != 0)[0]
        gt_boxes = np.empty((len(gt_inds), 9), dtype=np.float32)
        #print  ('roi_rec[boxes]=',roi_rec['boxes'][gt_inds,:])
        gt_boxes[:,0:8]=roi_rec['boxes'][gt_inds,:]
        #back_forward_convert(,True)
        #print ("gt_boxes_shape==",gt_boxes.shape)
        gt_boxes[:, 8] = roi_rec['gt_classes'][gt_inds] #gt_boxes的标签

        # scale gt_boxes
        gt_boxes[:, 0:8] *= im_scale  #只有坐标改变系数，label不变

    else:
        gt_boxes = np.empty((0, 9), dtype=np.float32)
    #print ("gt_boxes_data=",gt_boxes)
